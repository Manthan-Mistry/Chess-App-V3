...
def display_player_stats(df: pd.DataFrame, player: str) -> Tuple[int, float, float, int, int, int, int, float, float, float,
    ...
    if total_games_white > 0:
        ...
        white_win_ratio = 0.0
        white_loss_ratio = 0.0
        white_draw_ratio = 0.0
    ...
    total_games_black = len(df[(df['black_username'] == player)])
    if total_games_black > 0:
        ...
        black_win_ratio = 0.0
        black_loss_ratio = 0.0
        black_draw_ratio = 0.0
...
def optimized_get_player_stats_db_live(player_name: str, supabase_url: str, supabase_key: str) -> pd.DataFrame:
    ...
    for game in games:
        game_data = {
            ...
            "white_result": game.get("white", {}).get("result"),
            "white_username": game.get("white", {}).get("username").lower(),
            "white_accuracy": game.get("accuracies", {}).get("white", 0.0), # Make sure to give default value of 0.0 or will get data saving error because of null values.
            "black_rating": game.get("black", {}).get("rating"),
            "black_result": game.get("black", {}).get("result"),
            "black_username": game.get("black", {}).get("username").lower(),
...
def get_player_data_from_supabase(player_name: str, supabase_url: str, supabase_key: str) -> pd.DataFrame:
    """
    Fetches all data for a specific player from Supabase.

    Args:
        player_name (str): The username of the chess player.
        supabase_url (str): The Supabase project URL.
        supabase_key (str): The Supabase API key.

    Returns:
        pd.DataFrame: A DataFrame containing the player's data if found; otherwise, an empty DataFrame.
    """
    # Create a Supabase client
    supabase: Client = create_client(supabase_url, supabase_key)
    
    # Initialize variables for pagination
    all_rows = []  # List to store all rows
    batch_size = 1000  # Max rows per request
    start = 0  # Start index for pagination

    try:
        while True:
            # Query the Supabase database for the selected player with pagination
            ...
            all_rows.extend(response.data)  # Append the current batch to the list
            
            # If fewer than batch_size rows are returned, we have reached the end
        ...
        # If data was found, convert it to a DataFrame
        if all_rows:
            df = pd.DataFrame(all_rows)
            print(f"Data retrieved for player: {player_name}")
            return df
        else:
...
def delete_player_data(player_name: str, supabase_url: str, supabase_key: str):
    """
    Deletes all data for the specified player from the Supabase database.
    ...
    try:
        ...
        # Delete player data from the table
        response = supabase.table("player_game_data").delete().eq("player_name", player_name).execute()

        st.write(f"Data for player '{player_name}' successfully deleted.")
        
    except Exception as e:
        st.write(f"Error deleting player data: {e}")