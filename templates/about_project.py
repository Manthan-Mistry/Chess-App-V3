...
# Load SVG file
    with open("assets/db-diagram-1.svg", "r") as svg_file:
    ...
    st.markdown("""
        ...
        - **Database Implementation**:  Using Supabase database to fetch already existed player data reduces loading time significantly instead of fetching data directly 
        from API.
        - **Concurrent Fetching Requests**:  fetch_archive_urls and fetch_games use aiohttp for asynchronous HTTP requests, 
        allowing archives and games to be fetched simultaneously.
    """, unsafe_allow_html=True)
    ...
    st.markdown("""  
        ...
        - **Using Upsert instead of insert in supabase:** Using upsert ensures new records are inserted while existing ones are updated, preventing duplicate entries and maintaining data integrity. It simplifies workflows by combining the functionality of insert and update in a single operation, making it ideal for managing unique or primary key-constrained data.
        - **Batch Insert:** Batch insert improves performance by sending multiple records to the database in a single query, reducing the overhead of multiple insert operations avoiding'canceling statement due to statement timeout' error.
    """, unsafe_allow_html=True)
    ...
    st.write("""
        ...
        - **Supabase Database** for saving player data in live section. 
    """)
