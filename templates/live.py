        ...
        if st.session_state.button_pressed:
            ...
            with st.spinner():                
                if selected_player in all_players:
                    st.write("Player exists in Database. Getting Data...")
                    df = get_player_data_from_supabase(selected_player, supabase_url= SUPABASE_URL, supabase_key= SUPABASE_KEY)
                else:
                    st.write("Player doesn't exists in Database. Getting Data from API. Please Wait....")
                    df = optimized_get_player_stats_db_live(selected_player, supabase_url= SUPABASE_URL, supabase_key= SUPABASE_KEY)
